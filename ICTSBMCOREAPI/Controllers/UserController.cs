            ...
            if (Auth_AppId == AppId)
            {
                try
                {
                    ...
                    if (GIS_CON != null)
                    {
                        ...
                        //Start New Code.
                        //client.BaseAddress = new Uri("http://114.143.244.130:9091/house/all");
                        client.BaseAddress = new Uri(GIS_CON.url);
                        //client.DefaultRequestHeaders.Accept.Clear();
                        client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                        client.DefaultRequestHeaders.Add("url", gis_url + "/" + gis_DBName);
            ...
            if (Auth_AppId == AppId)
            {
                try
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(db.Database.GetDbConnection().ConnectionString))
                    {
                        ...
                        const string CheckIfTableExistsStatement = "SELECT * FROM sys.objects WHERE name = N'GetHouseTrail'";
                        command.CommandText = CheckIfTableExistsStatement;
                        var executeScalar = command.ExecuteScalar();
                        if (executeScalar != null)
                        {

                            string sprocname = "GetHouseTrail";
